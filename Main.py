...
def mthSplitInit(FileName):
	...
	while True:
		...
		for Line in Lines:
			if Count==2:
				FileHandle_Base64.write(Line.encode())
			else:
				if Line[0]=='-' and Line[1]=='-' and Line[2]=='-':
					...
					pass
				if Count==0:
					FileHandle_Head.write(Line.encode())
				else:
					if Count==1:
						FileHandle_Message.write(Line.encode())
					else:
						pass
	FileHandle.close()
...
def SplitMessage(FileName,MaxBlock):
	...
	FileHandle_Message=open(FileName,encoding="UTF-8")
	WriteCount=0
	FileCounter=0
	OutFileHandle=open(CreatFolder(OutFolder)+GetFileName(FileName).replace('.mht','')+'_Split_'+MakeString(FileCounter+1,3)+'.html','wb')
	OutFileHandle.write(
		'''<html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" /><title>QQ Message</title><style type="text/css">body{font-size:12px; line-height:22px; margin:2px;}td{font-size:12px; line-height:22px;}</style></head><body><table width=100% cellspacing=0>'''.encode()
		)
	while True:
		...
		for Line in Lines:
			if WriteCount<MaxBlock:
				OutFileHandle.write(Line.replace('IMG src="','IMG src="'+TrueImageFolder).encode())
			else:
				OutFileHandle.close()
				FileCounter+=1
				OutFileHandle=open(CreatFolder(OutFolder)+GetFileName(FileName).replace('.mht','')+'_Split_'+MakeString(FileCounter+1,3)+'.html','wb')
